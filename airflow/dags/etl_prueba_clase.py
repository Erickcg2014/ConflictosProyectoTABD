...
S3_KEY = os.getenv("S3_KEY", "CSV_DATA/conflictos_prueba_clase.csv")
...
def extract_and_transform_data(**kwargs) -> Dict[str, Any]:
    ...
    # Imprimir informaciónsobre las columnas
    ...
    if missing_cols:
        print(f"  ADVERTENCIA: Columnas faltantes en el CSV: {missing_cols}")
...
def load_to_mongodb(**kwargs):
    ...
    print(f"🔄 Transformando {len(df):,} registros a GeoJSON...")
    ...
    for _, row in df.iterrows():
        ...
        país_a = row.get("país_a") if row.get("país_a") is not None else None
        país_b = row.get("país_b") if row.get("país_b") is not None else None
    ...
    print(f"📤 Preparando {len(operations):,} operaciones...")

    try:
        result = collection.bulk_write(operations, ordered=False)
    except Exception as e:
        print("❌ ERROR EN BULK WRITE")
        print(e)
        raise
...
# DAG
with DAG(
    dag_id="etl_prueba_clase",
    start_date=pendulum.datetime(2023, 1, 1, tz="UTC"),
    schedule_interval="@once",
    catchup=False,
    is_paused_upon_creation=False,
    tags=["wars", "etl", "prueba", "2025"],
